...
class Array{
    ...
    Array(std::string name, int size): size(size), name(name) { // Initialization list
        this->size = size;
        this->name = name;
        array = new int[size];
    ...
    void printArray() const {
        std::cout << "Array name: " << name << " (size: " << size << ")" << std::endl;
        for(int i = 0; i < size; i++){
            std::cout << array[i] << " ";
        }
        std::cout << std::endl;
...
int main(){
    Array arr1("array1", 5); // Create an array of size 5
    arr1.printArray();
    ...
    arrays.push_back(std::move(arr1)); // move constructor is called
    arrays.push_back(std::move(arr2));
    arrays.push_back(std::move(arr3));
    arrays.push_back(std::move(arr4)); // move constructor is called
